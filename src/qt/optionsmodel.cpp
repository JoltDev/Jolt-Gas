...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeJoltGasAmount"))
        settings.setValue("nAnonymizeJoltGasAmount", 1000);
    nAnonymizeJoltGasAmount = settings.value("nAnonymizeJoltGasAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeJoltGasAmount"))
        SoftSetArg("-anonymizejoltgasamount", settings.value("nAnonymizeJoltGasAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeJoltGasAmount:
            return QVariant(nAnonymizeJoltGasAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeJoltGasAmount:
            nAnonymizeJoltGasAmount = value.toInt();
            settings.setValue("nAnonymizeJoltGasAmount", nAnonymizeJoltGasAmount);
            emit AnonymizeJoltGasAmountChanged(nAnonymizeJoltGasAmount);
            break;
        default:
            break;
        }
