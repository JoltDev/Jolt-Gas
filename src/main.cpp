...
const string strMessageMagic = "JoltGas Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    else if (nHeight > 1 && nHeight < 390000) {
        nSubsidy = 25 * COIN;
    }
    else if (nHeight > 390000) {
        nSubsidy = 25 * COIN;
        nSubsidy >>= ((nHeight + 390000) / 390000);
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest->nHeight+1 > 1500 && pindexBest->nHeight+1 <= 390000)  {
        nSubsidy = 25 * COIN;
    } else if (pindexBest->nHeight+1 > 390000) {
        nSubsidy = 25 * COIN;
        nSubsidy >>= ((pindexBest->nHeight + 390000) / 390000);
    }

    return nSubsidy + nFees;
}

static int64_t nTargetTimespan = 5 * 150;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight < 1500) {
	ret = 0;
    } else if (nHeight >= 1500 && nHeight <= 390000) {
